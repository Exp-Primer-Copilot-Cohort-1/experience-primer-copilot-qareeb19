 ...
 */
   ...
   function compareCountries(data, country1, country2, year) {
    const reducer = (acc, curr) => {
      if ((curr.country === country1 || curr.country === country2) && curr.year === year) {
        acc.push(curr);
      }
      return acc;
    };
...
function largestEmitterForYear(data, year) {
  const dummy = { country: '', year: year, annual_emissions: 0 }
  const reducer = (acc, curr) => {
    if (curr.year == year) {
      if (curr.annual_emissions > acc.annual_emissions) {
        return curr;
      }
    }
    return acc;
  }
  return data.reduce(reducer, dummy);
}
...
Transaction.storeDetails = function(details) {
  storeVal.push(details);
  // Make sure to freeze the array again if needed.
...
function promiseAll(promisesArr) {
  ...
  return new Promise((resolve, reject) => {
    if (!promisesArr.length) resolve(results);
    promisesArr.forEach((promise, index) => {
      Promise.resolve(promise).then(result => {
        results[index] = result;
        completedPromises += 1;
        if (completedPromises === promisesArr.length) {
          resolve(results);
        }
      }).catch(reject);
    });
...
function unreliableCallProm(recipient, probability = Math.random()) {
  return new Promise((resolve, reject) => {
    if (recipient === null || recipient === undefined || typeof recipient !== 'string') {
      ...
      const waitTime = Math.max(400, Math.floor(Math.random() * (10000 - 4000 + 1) + 4000));
      setTimeout(() => {
        resolve(`Call to ${recipient} was delayed by ${waitTime / 1000} seconds`);
      }, waitTime);
    }
  });
}
...
function shouldWaitForCall(recipient, probability) {
  return new Promise((resolve, reject) => {
    unreliableCallProm(recipient, probability).then(result => {
      if (result.includes('delayed by') && parseFloat(result.split('delayed by ')[1].split(' ')[0]) > 3) {
        reject(false);
      } else {
        resolve(true);
      }
